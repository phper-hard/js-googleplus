...
//生成图片存放路径
$new_avatar_path = 'uploads/avatar_'.$type.'/'.$pic_id.'_'.$type.'.jpg';
$new_avatar_path_big = 'uploads/avatar_big/'.$pic_id.'_'.'big.jpg';//显示图片
...
$d->status = 1;
$d->statusText = '上传成功!';
setcookie("avatar_url",$new_avatar_path_big, time()+3600*24);
$msg = json_encode($d);
...
function  log_result($word) { 
	@$fp = fopen("log/log.txt","a");	//写入日志文件 以追加模式
	@flock($fp, LOCK_EX) ;
